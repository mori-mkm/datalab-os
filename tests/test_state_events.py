...
def test_event_schema_roundtrip_and_validation():
    bus = EventBus()
    event = bus.emit("r1", EventType.agent_started, department="analytics", agent="analytics_lead", status="running", message="hi")
...
def test_bus_orders_events_and_replays_then_streams_live():
    ...
    bus.emit("r1", EventType.agent_started, department="a")
    received: list[int] = []
